...
def harmonic_comparison(initial_angle1, t_max=30):
    t = np.linspace(0, t_max, 500)
    theta1 = solve_ivp(double_pendulum_derivs, (0, t_max), [initial_angle1, 0, initial_angle1, 0], t_eval=t).y[0]

    w = minimize_scalar(lambda w: np.sum((theta1 - initial_angle1*np.cos(w*t))**2), bounds=(0.1, 15), method="bounded").x
...
def main():
    ...
    if args.animate:
        ...
        print(f"RMS {harmonic_comparison(angle)[2]}")
        animate_comparison(angle)
    else:
        plot_rms()
